...
4. **MODIFY POTENTIAL FILE**: Update potential.mod with the correct potential found in workdir
...
5. **RUN SIMULATION**: Execute the LAMMPS simulation with in.elastic file on HPC
...
6. **EXTRACT MATRIX**: Get the 6x6 elastic constants matrix from output
...
7. **COMPARE RESULTS**: Check matrix values for consistency and symmetry
...
Always start by checking if template files exist, then identify available potential files in the working directory. Always used the relaxed structure for the calculations.
